...
| **pickupType** | `"staff"` \| `"family"` | ✓ | `"staff"` | 朝の送迎タイプ（職員送迎 or 家族送迎） |
| **dropoffType** | `"staff"` \| `"family"` | ✓ | `"staff"` | 夕方の送迎タイプ（職員送迎 or 家族送迎） |
...
#### pickupType / dropoffType（送迎タイプ）

**目的**: 送迎車両の配車計画、新規受入判断のため
...
**業務上の重要性**:
1. **送迎車両の配車計画**: 職員送迎の人数から必要な車両数を算出
2. **新規受入判断**: 送迎車両が満車の場合、家族送迎可能な利用者を優先
3. **日別サマリーでの表示**: 職員送迎が必要な人数を迎え・送りで表示

**Phase 1での実装**:
- 両方とも `"staff"` がデフォルト
- Phase 1では入力UIは未実装（全員職員送迎と仮定）
- Phase 1.5以降で入力UIを追加予定
...
class KayoiSchedule {
  ...
  validate() {
    ...
    // 送迎タイプのバリデーション
    if (!this.isValidPickupType(this.pickupType)) {
      return { valid: false, error: 'pickupTypeが不正です（"staff"または"family"）' };
    }
    if (!this.isValidDropoffType(this.dropoffType)) {
      return { valid: false, error: 'dropoffTypeが不正です（"staff"または"family"）' };
    }
    return { valid: true };
  }
  ...
  isValidPickupType(pickupType) {
    return ['staff', 'family'].includes(pickupType);
  }

  isValidDropoffType(dropoffType) {
    return ['staff', 'family'].includes(dropoffType);
  }
