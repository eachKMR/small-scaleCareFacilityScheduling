...
**例2: 未割当**:
```javascript
// 鈴木さんの泊まり期間は決まっているが、まだ部屋が決まっていない
{
  id: "tomari_002",
  userId: "user_suzuki",
  roomId: null,  // 未割当
...
| **roomId** | `string \| null` | - | `null` | 居室ID（例: `"room01"`）。`null`は未割当を表す |
...
```javascript
// 例1: 通いUIから泊まり期間を設定（未割当）
function handleLongPressCalendar(userId, startDate, endDate) {
  // 未割当で作成
...
// 例2: 泊まりUIで部屋割り当て
function handleRoomAssignment(roomId, startDate, endDate, userId) {
  // 部屋を指定して作成
  const reservation = new TomariReservation({
    userId: userId,
    roomId: roomId,  // 居室指定
...
| 2025-12-31 | 2.0 | roomIdをnull許可に変更、未割当機能追加 | Claude |
